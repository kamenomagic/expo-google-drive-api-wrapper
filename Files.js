...
export default class Files {
  ...
  async getId(name, parents, mimeType, trashed = false) {
    ...
    let result = await this.list({
      q:
        GDrive._stringifyQueryParams(queryParams, "", " and ", true) +
  ...
  async download(
    ...
    return await FileSystem.downloadAsync(
      `${GDrive._urlFiles}/${fileId}${parameters}`,
      FileSystem.documentDirectory + nameFileAndExtension,
      {
        ...downloadFileOptions,
      }
    );
  ...
  list(queryParams) {
    return fetch(
      `${GDrive._urlFiles}${GDrive._stringifyQueryParams(queryParams)}`,
      {
        headers: GDrive._createHeaders(),
      }
    );
  ...
  update(fileId, queryParams) {
    ...
    return fetch(
      `${GDrive._urlFiles}/${fileId}${GDrive._stringifyQueryParams(
        queryParams
      )}`,
      config
    );
